...
const AddItems: React.FC<{ open: boolean; onClose: () => void }> = ({
	open,
	onClose,
}) => {
	const user = useUser();
	...
	const onSubmit = async (data: any) => {
		...
		const res = await handleResponse(
			() =>
				createShipment({
					...
					product: pid,
					stitch:
						data.stitch?.size || data.stitch?.fee ? data.stitch : undefined,
		...
		message.destroy();
		if (res.status) {
			message.success("Added items successfully!");
			reset();
			reset({
				supplier: null,
				product: null,
			...
			>
				<form onSubmit={handleSubmit(onSubmit)}>
					<DialogTitle className={"flex flex-row items-center justify-between"}>
						<ListItemText
							primary={"Item"}
							secondary={`Add items to product`}
					...
					<Divider />
					<DialogContent>
						{/* <div className="grid grid-cols-1 sm:grid-cols-1 gap-2"></div> */}
