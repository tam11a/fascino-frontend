...
import { useUpdateBranch } from "@/queries/branch";
...
const Info: React.FC = () => {
  ...
  const { data: supplierhData } = useGetSuppliersById(sid);
  const { mutateAsync: updateSupplier } = useUpdateSupplier();

  React.useEffect(() => {
    if (!supplierhData) return;
    reset({
      name: supplierhData?.data?.data?.name,
      phone: supplierhData?.data?.data?.phone,
      address: supplierhData?.data?.data?.address,
    });
  }, [supplierhData]);
